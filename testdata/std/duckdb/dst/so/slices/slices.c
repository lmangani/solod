...
static uint64_t nextPowerOfTwo(so_int length);
...
void slices_SortWith(slices_Sorter s) {
    so_int limit = bits_Len((uint64_t)(s.slice.len));
...
static uint64_t nextPowerOfTwo(so_int length) {
    uint64_t shift = (uint64_t)(bits_Len((uint64_t)(length)));
    return (uint64_t)((uint64_t)1 << shift);
}
...
static bool partialInsertionSort_func(slices_Sorter data, so_int a, so_int b) {
    const so_int maxSteps = 5;
    const so_int shortestShifting = 50;
    so_int i = a + 1;
...
static void breakPatterns_func(slices_Sorter data, so_int a, so_int b) {
    ...
    if (length >= 8) {
        ...
        uint64_t modulus = nextPowerOfTwo(length);
        for (so_int idx = a + (length / 4) * 2 - 1; idx <= a + (length / 4) * 2 + 1; idx++) {
            so_int other = (so_int)((uint64_t)(xorshift_Next(&random)) & (modulus - 1));
            if (other >= length) {
                other -= length;
            }
...
// Return (pivot, hint)
static so_R_int_int choosePivot_func(slices_Sorter data, so_int a, so_int b) {
    const so_int shortestNinther = 50;
    const so_int maxSwaps = 4 * 3;
    so_int l = b - a;
    so_int swaps = 0;
    so_int i = a + l / 4 * 1;
    so_int j = a + l / 4 * 2;
...
static void symMerge_func(slices_Sorter data, so_int a, so_int m, so_int b) {
    ...
    if (m - a == 1) {
        ...
        so_int j = b;
        for (; i < j;) {
            so_int h = (so_int)((uint64_t)(i + j) >> 1);
    ...
    so_int mid = (so_int)((uint64_t)(a + b) >> 1);
    so_int n = mid + m;
    so_int start = 0, r = 0;
    if (m > mid) {
        start = n - b;
    ...
    so_int p = n - 1;
    for (; start < r;) {
        so_int c = (so_int)((uint64_t)(start + r) >> 1);
